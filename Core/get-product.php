...
$query = "select ProductId, Name, Info, CurrentPrice, CurrentUser, OwnerId, Duration, ProductImg, Username, IsPrgStatus, TotalBet, Type, CreatedDate, StartTime, ExpireTime from vproducts where IsPrgStatus != 4 Order by IsPrgStatus asc, CreatedDate desc limit 100";
if (isset($_GET['Id'])){
	$id = $_GET['Id'];
...
if (isset($_GET['CurrentUser'])) {
	...
	$query = "select ProductId, Name, Info, CurrentPrice, CurrentUser, OwnerId, Duration, ProductImg, Username, IsPrgStatus, TotalBet, Type, CreatedDate, StartTime, ExpireTime from vproducts where IsPrgStatus != 4 and CurrentUser = '$user' Order by CreatedDate";
}
